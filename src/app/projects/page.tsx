        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-16 opacity-0 animate-fill-forwards animate-fade-in-up">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400 max-w-3xl leading-relaxed">
              Flagship research projects advancing the science of behavioral digital attribution and cybersecurity intelligence.
            </p>
          </div>

          <div className="space-y-8">
            {projects.map((project, i) => (
              <div key={project.title} className="group p-8 lg:p-10 rounded-2xl bg-gradient-to-br from-primary-50 to-accent-50 dark:from-primary-900/20 dark:to-accent-900/20 border border-primary-200 dark:border-primary-800 hover:shadow-xl transition-all opacity-0 animate-fill-forwards animate-fade-in-up" style={{ animationDelay: `${i * 0.1}s` }}>
                <div className="flex items-start gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-accent-100 dark:bg-accent-900/30 text-accent-700 dark:text-accent-300">{project.status}</span>
                    </div>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{project.role}</p>
                    <p className="text-slate-700 dark:text-slate-300 leading-relaxed mb-6">{project.description}</p>
                    <div className="flex flex-wrap gap-3">
                      <a href={project.links.github} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-medium hover:bg-slate-800 dark:hover:bg-slate-100 transition-colors">
                        <Github size={16} /> GitHub <ExternalLink size={14} />
                      </a>
                      <a href={project.links.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                        Website <ExternalLink size={14} />
                      </a>
                      <a href={project.links.doi} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
          ...
          <div className="mt-12 text-center opacity-0 animate-fill-forwards animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            <Link href="/anubisx/" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-medium transition-colors shadow-lg shadow-primary-500/25">
              Explore AnubisX Framework in Detail <ArrowRight size={18} />
            </Link>
          </div>
        </div>
      </section>
