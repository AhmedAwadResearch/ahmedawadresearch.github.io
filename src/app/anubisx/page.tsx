...
const highlights = [
  ...
  {
    icon: BookOpen,
    title: "Cognitive Centroid Theory",
    description:
...
const modalities = [
  {
    ...
    description: "Linguistic style analysis, authorship attribution, writeprints",
    algorithms: "ALG-001 to ALG-004",
  },
  {
    name: "Chrono-Profiling",
    status: "Specified",
    description: "Temporal behavior patterns, activity rhythms, posting schedules",
    algorithms: "ALG-005 to ALG-008",
  },
  {
    name: "Terminal Profiling",
    status: "Specified",
    description: "Command-line patterns, shell behavior, tool usage signatures",
    algorithms: "ALG-009 to ALG-012",
  },
  {
    name: "Network Analysis",
    status: "Specified",
    description: "Social graph analysis, interaction patterns, community detection",
    algorithms: "ALG-013 to ALG-016",
  },
  {
    name: "Media Forensics",
    status: "Specified",
    description: "Image metadata, editing artifacts, content analysis",
    algorithms: "ALG-017 to ALG-020",
  },
];
          ...
          >
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <a
                ...
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-medium hover:bg-slate-800 dark:hover:bg-slate-100 transition-colors"
              >
                <Github size={18} />
                GitHub Repository
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {highlights.map((item, i) => (
              <motion.div
                ...
                variants={fadeInUp}
                custom={i}
                className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-primary-300 dark:hover:border-primary-700 transition-all hover:shadow-lg"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400 max-w-3xl mx-auto">
              The framework implements a six-layer architecture with a six-stage pipeline supporting identification,
              verification, and forensic comparison workflows.
            </p>
          </motion.div>
          <div className="grid gap-4 max-w-3xl mx-auto">
            {architectureLayers.map((layer, i) => (
              <motion.div
                ...
                variants={fadeInUp}
                custom={i}
                className="flex items-center gap-4 p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700"
              >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {modalities.map((mod, i) => (
              <motion.div
                ...
                variants={fadeInUp}
                custom={i}
                className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-primary-300 dark:hover:border-primary-700 transition-all"
              >
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-slate-900 dark:text-white">{mod.name}</h3>
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    mod.status === "Prototype Validated"
                      ? "bg-accent-100 dark:bg-accent-900/30 text-accent-700 dark:text-accent-300"
                      : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400"
                  }`}>
                    {mod.status}
